    ...
    <div className="min-h-[calc(100vh-56px)] flex items-center justify-center px-4 py-12">
      <motion.div className="w-full max-w-[400px]" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <div className="text-center mb-8">
          ...
          <h1 className="font-display text-3xl text-[var(--color-ink)] italic mb-2">Selamat Datang</h1>
          <p className="text-sm text-[var(--color-ink-muted)]">Masuk untuk melanjutkan perjalananmu</p>
        </div>

        <div className="map-card p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            {error && (
              <div className="p-3 rounded border border-[var(--color-stamp-red)]/30 bg-[var(--color-stamp-red)]/5 text-sm text-[var(--color-stamp-red)] font-body">
                {error}
              </div>
            )}
            <div>
          ...
          <p className="text-center text-sm text-[var(--color-ink-muted)] mt-5">
            Belum punya akun?{' '}
            <Link to="/register" className="font-semibold text-[var(--color-ocean)] hover:underline">Daftar</Link>
          </p>
        </div>
      </motion.div>
    </div>
