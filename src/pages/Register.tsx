    ...
    <div className="min-h-[calc(100vh-56px)] flex items-center justify-center px-4 py-12"
      ...
      <motion.div className="w-full max-w-[400px]" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <div className="text-center mb-8">
          ...
          <h1 className="font-display text-3xl font-extrabold mb-2" style={{ color: '#FFF6DD' }}>Mulai Perjalanan</h1>
          <p className="text-sm" style={{ color: 'rgba(255,246,221,0.6)' }}>Buat akun dan mulai belajar AI</p>
        </div>

        <div style={{ background: 'var(--sand)', border: '2px solid var(--wood-dark)', borderRadius: 16, padding: 24 }}>
          <form onSubmit={handleSubmit} className="space-y-4">
            {error && (
              <div className="p-3 rounded-lg text-sm font-bold" style={{ background: '#FBE2E0', color: 'var(--flag)', border: '1px solid var(--flag)' }}>{error}</div>
            )}
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider mb-1.5" style={{ color: 'var(--ink-soft)' }}>Username</label>
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider mb-1.5" style={{ color: 'var(--ink-soft)' }}>Konfirmasi Password</label>
          ...
          <p className="text-center text-sm mt-5" style={{ color: 'var(--ink-soft)' }}>
            Sudah punya akun?{' '}
            <Link to="/login" className="font-bold" style={{ color: 'var(--ocean)' }}>Masuk</Link>
          </p>
        </div>
      </motion.div>
    </div>
